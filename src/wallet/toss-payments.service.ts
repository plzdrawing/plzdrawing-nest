...
type TossConfirmResponse = TossPaymentResponse;
type TossCancelResponse = TossPaymentResponse;
...
export class TossPaymentsService {
  ...
  async cancelPayment(
    ...
    const response = await fetch(
      `${baseUrl}/v1/payments/${encodeURIComponent(paymentKey)}/cancel`,
      {
        method: 'POST',
        headers: {
          Authorization: `Basic ${authValue}`,
    ...
    return body as TossCancelResponse;
  }
  ...
  async getPayment(paymentKey: string): Promise<TossPaymentResponse> {
    ...
    const authValue = Buffer.from(`${secretKey}:`).toString('base64');
    const response = await fetch(
      `${baseUrl}/v1/payments/${encodeURIComponent(paymentKey)}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Basic ${authValue}`,
